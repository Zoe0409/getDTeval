...
```{r benchmark.getDTeval_example, eval=FALSE, include=FALSE}
the.statement <- "dat[get(age.name) > 65, .(mean_awareness = mean(get(awareness.name))), keyby = c(eval(gender.name), eval(region.name))]"
...
```{r getDTeval_example1, eval=FALSE, include=FALSE}
