...
This is a benchmarking experiment on data with about 1 million rows. The result shows the reduction in running time using getDTeval over the original statement.

```{r benchmark.getDTeval_example1-1}
dat <- snack.dat[sample(x = 1:.N, size = 10^6, replace = T)]
```

```{r benchmark.getDTeval_example1-2}
the.statement <- "dat[get(age.name) > 65, .(mean_awareness = mean(get(awareness.name))), keyby = c(eval(gender.name), region.name)]"
```

```{r benchmark.getDTeval_example1-3}
...
getDTeval(the.statement = "snack.dat[, .(eval(mean.age.name) = mean(get(age.name)))]")
```
...
getDTeval(the.statement = "snack.dat %>% summarize(eval(mean.age.name) = mean(get(age.name)))")
```
